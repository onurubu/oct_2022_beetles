...

{
  ...
  comm <- abund_all %>% mutate(veg_type = factor(case_when(site %in% c(1) ~ "strandveld", site %in% c(2, 6, 16) ~ "restioid", site %in% c(3, 4, 5) ~ "proteoid", site %in% c(7, 8, 9, 10, 12, 13, 14, 15) ~ "ericaceous", site %in% c(11) ~ "alpine", site %in% c(17) ~ "succulent_karoo"))) %>% relocate(site, replicate, year, veg_type) %>% group_by(year, season, period, veg_type, site) %>% summarise(across(where(is.numeric), sum), .groups = "drop") %>% relocate(year, season, period, veg_type, site) %>% arrange(site, year)
  ...
  comm$aspect <- gsub("\\)", "", comm$aspect)
  comm$site <- as.numeric(sapply(strsplit(comm$site, "m", fixed = TRUE), "[[", 1))
...
write.csv(summary_table, "./range_shifts/elevational_shift_summary.csv", row.names = FALSE)
...
p_all <-ggplot(spp_long,
  ...
  scale_x_discrete(labels = c("anthia_decemguttata" = "Anthia decemguttata", "stenocara_dentata" = "Stenocara dentata", "zophosis_gracilicornis" = "Zophosis gracilicornis"))

ggsave(paste0("./range_shifts/all_slopes_shifts.png"), plot = p_all, width = 3840, height = 2160, units = "px", bg = "white", create.dir = TRUE)

rm(dat, dat02, dat22, tmp, tmp02, tmp22, asp.names, i, maxes, mins, x, sampling, spp_long, spp.elev, count_dir)
...
write.csv(not_plotted_east, "./range_shifts/not_plotted_east.csv", row.names = FALSE)
write.csv(not_plotted_west, "./range_shifts/not_plotted_west.csv", row.names = FALSE)
write.csv(not_plotted_both, "./range_shifts/not_plotted_both_slopes.csv", row.names = FALSE)
...
ggsave("./range_shifts/West_slope_shift.png", p_west, width = 3840, height = 2160, units = "px", bg = "white", create.dir = TRUE)
ggsave("./range_shifts/East_slope_shift.png", p_east, width = 3840, height = 2160, units = "px", bg = "white", create.dir = TRUE)

rm(dat, dat02, dat22, tmp, tmp02, tmp22, asp.names, i, maxes, mins, x, sampling, spp_long, spp.elev, count_dir, p_all, p_east, p_west, no_shift_both, no_shift_east, not_plotted_west, no_shift_west, not_plotted_both, not_plotted_east, spp_long_filt)
rm(shift_summary, summary_table)
...

{
  ...
  comm <- beet_wide %>% mutate(veg_type = factor(case_when(site %in% c(1) ~ "strandveld", site %in% c(2, 6, 16) ~ "restioid", site %in% c(3, 4, 5) ~ "proteoid", site %in% c(7, 8, 9, 10, 12, 13, 14, 15) ~ "ericaceous", site %in% c(11) ~ "alpine", site %in% c(17) ~ "succulent_karoo"))) %>% relocate(site, replicate, year, veg_type) %>% group_by(year, season, veg_type, site) %>% summarise(across(where(is.numeric), sum), .groups = "drop") %>% relocate(year, season, veg_type, site) %>% arrange(site, year)
  comm <- comm %>% mutate(site = alt_labels[comm$site])
  comm[1:6, 1:6]
  comm$aspect <- sapply(strsplit(comm$site, "m", fixed = TRUE), "[[", 2)
  comm$aspect <- gsub("(Peak)", "W", comm$aspect)
...
write.csv(summary_table, "./range_shifts/modern_seasonal/elevational_shift_summary.csv", row.names = FALSE)
...
ggsave(paste0("./range_shifts/modern_seasonal/all_slopes_shifts.png"), plot = p_all, width = 3840, height = 2160, units = "px", bg = "white", create.dir = TRUE)
...
write.csv(not_plotted_east, "./range_shifts/modern_seasonal/not_plotted_east.csv", row.names = FALSE)
write.csv(not_plotted_west, "./range_shifts/modern_seasonal/not_plotted_west.csv", row.names = FALSE)
write.csv(not_plotted_both, "./range_shifts/modern_seasonal/not_plotted_both_slopes.csv", row.names = FALSE)
...
p_west   # print West plot
ggsave("./range_shifts/modern_seasonal/West_slope_shift.png", p_west, width = 3840, height = 2160, units = "px", bg = "white", create.dir = TRUE)
ggsave("./range_shifts/modern_seasonal/East_slope_shift.png", p_east, width = 3840, height = 2160, units = "px", bg = "white", create.dir = TRUE)
