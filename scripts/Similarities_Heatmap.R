...
dat <- abund_all %>%
  ...
  group_by(period, year, season, site, replicate, veg_type) %>%
...
dat[, species_cols] <- lapply(dat[, species_cols], function(x) {
  ifelse(x > 0, 1, 0)
})
...
# ==========================================
dat_agg <- aggregate(. ~ period + `site`, data = dat, FUN = max)
...
for (i in seq_len(nrow(mat_2002))) {
  for (j in seq_len(nrow(mat_2022))) {
    sim_mat[i, j] <- 1 -
      vegdist(rbind(mat_2002[i, ], mat_2022[j, ]), method = "jaccard")
  }
}
...
heatmap_plot <- ggplot(
  heat_df,
  aes(x = Site_2022, y = Site_2002, fill = Similarity)
) +
  ...
  labs(x = "2022–2023", y = "2002–2003", title = "Jaccard Similarity Heatmap") +
...
for (site in elev_levels) {
  if (!(site %in% rownames(dat_2002)) || !(site %in% rownames(dat_2022))) {
    next
  }
  ...
  if (length(sp_2002) == 0 & length(sp_2022) == 0) {
    next
  }
...
cat(
  "All plots (heatmap, dendrograms, and Venn diagrams) saved to Heatmap_and_Venn.pdf\n"
)
