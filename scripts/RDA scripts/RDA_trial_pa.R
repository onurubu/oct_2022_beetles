...
# ==========================================
spp_data <- decostand(spp_data_all[, -c(1:5)], method = "pa") # remove non-species columns if needed
...
# Forward selection of variables:
{
  fwd.sel <- ordistep(
    ...
    scope = formula(model_full), # upper model limit (the "full" model)# upper model limit (the "full" model)
    direction = "forward",
