...
# Import data
# ==========================================
env_data_all <- env_season %>% filter(year == 2022)
spp_data_all <- abund_seasonal %>% filter(year == 2022)
...
Zenv_data <- Zenv_data %>%
  ...
  select(-AMin_precip, -mMin_precip)
...
{
  spe.rda.signif <- rda(
    formula = Zspp_data ~ Name + d_fire + mMax_tground + litter,
    data = Zenv_data
  )
